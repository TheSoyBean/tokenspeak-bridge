#!/usr/bin/env python3
"""
train-bridge.py — LoRA fine-tune llama3.1:8b for tokenspeak bridge layer.

Forked from train-null.py (same dir, proven pattern).
Trains L1→L2 translation: keyword sequences → operator+symbol sequences.

Strategy: Train LoRA against vanilla llama3.1:8b-instruct (full vocab, stable gradients).
At inference: merge LoRA adapter onto xar-pruned model via ollama Modelfile ADAPTER directive.

Dataset: split-chat/train-bridge-chat.jsonl (cleaned, chat-formatted)
Output:  bridge-lora-output/

Usage:
    .venv/bin/python train-bridge.py
"""
...
DATASET = (
    Path.home()
    / "Obsidian-Vault/lattice/training/split-chat/train-bridge-chat.jsonl"
)
OUTPUT = Path(__file__).parent / "bridge-lora-output"
LORA_R = 8
LORA_ALPHA = 16
EPOCHS = 5
BATCH_SIZE = 1
...
def main():
    ...
    try:
        ...
        print("Create Ollama model: ollama create bridge-lora -f Modelfile.bridge-lora")
