#!/usr/bin/env python3
"""
train-translator.py — LoRA fine-tune llama3.1:8b for translator layer (L0→L1).

Forked from train-bridge.py (same dir, proven pattern).
Trains L0→L1 translation: natural English → compressed keywords.

Strategy: Train LoRA against vanilla llama3.1:8b-instruct (full vocab, stable gradients).
At inference: merge LoRA adapter onto base model via ollama Modelfile ADAPTER directive.

Dataset: split-chat/train-translator-chat.jsonl (cleaned, chat-formatted)
Output:  translator-lora-output/

Usage:
    .venv/bin/python train-translator.py
"""
...
DATASET = (
    Path.home()
    / "Obsidian-Vault/lattice/training/split-chat/train-translator-chat.jsonl"
)
OUTPUT = Path(__file__).parent / "translator-lora-output"
LORA_R = 8
LORA_ALPHA = 16
EPOCHS = 5
BATCH_SIZE = 1
...
def main():
    ...
    if not DATASET.exists():
        ...
        print("Run convert-to-chat.py first.")
        return
    ...
    try:
        ...
        print("Create Ollama model: ollama create translator-lora -f Modelfile.translator-lora")
